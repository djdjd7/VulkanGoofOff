...
void App::createWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "vulkan goof off", nullptr, nullptr);
...
VkSurfaceFormatKHR App::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
    ...
    for (const auto& availableFormat : availableFormats) {
        ...
        candidates.insert(std::make_pair(score, availableFormat));
    }

    if (candidates.rbegin()->first > 0) {
        return candidates.rbegin()->second;
    } else {
        throw std::runtime_error("failed to choose a suitable swap surface format!");
    }
}
...
int App::rateDeviceSuitability(VkPhysicalDevice device) {
    ...
    if (!deviceFeatures.geometryShader
        ...
        || !extensionsSupported
        || !swapChainAdequate
        || !deviceFeatures.samplerAnisotropy) return 0;
...
void App::pickPhysicalDevice() {
    ...
    if (candidates.rbegin()->first > 0) {
        ...
        #ifndef NDEBUG
        std::cout << "Selected GPU " << deviceProperties.deviceID << ": " << deviceProperties.deviceName << ", type: " << deviceProperties.deviceType << "\n";
        #endif
...
VKAPI_ATTR VkBool32 VKAPI_CALL App::debugCallback(
    ...
    for (int i = 22 - message.length(); i != 0; i--) {
        message += " ";
    }
...
void App::createInstance() {
    ...
    appInfo.pApplicationName = "vulkan goof off";
    appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
    appInfo.pEngineName = "No Engine";
    appInfo.engineVersion = VK_MAKE_VERSION(1, 0, 0);
    appInfo.apiVersion = VK_API_VERSION_1_0;

    // make instance creation info
